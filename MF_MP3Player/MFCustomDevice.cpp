...
void MFCustomDevice::attach(uint16_t adrPin, uint16_t adrType, uint16_t adrConfig, bool configFromFlash)
{
    ...
    ********************************************************************************** */
    ...
    if (strcmp(parameter, "MF_MP3Player") == 0)
        _customType = MY_MP3PLAYER;


    if (_customType == MY_MP3PLAYER) {
        /* **********************************************************************************
            Check if the device fits into the device buffer
        ********************************************************************************** */
        if (!FitInMemory(sizeof(MF_MP3Player))) {
        ...
        ********************************************************************************** */
        ...
        _myMP3Player = new (allocateMemory(sizeof(MF_MP3Player))) MF_MP3Player(_pin1, _pin2);
        _myMP3Player->attach(Parameter1, Parameter2);
...
void MFCustomDevice::set(int16_t messageID, char *setPoint)
{
    ...
    if (_customType == MY_MP3PLAYER) {
        _myMP3Player->set(messageID, setPoint);
    }
}
